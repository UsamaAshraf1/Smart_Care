...
export default function DepartmentPage({ params }) {
  const departments = {
    ...
    neurology: {
      ...
      image: "/placeholder.svg?height=400&width=800",
      services: ["Neurological Consultation and Evaluation", "Electroencephalogram (EEG)", "Electromyography (EMG)", "Nerve Conduction Studies"],
      doctors: [
        {
          image: "/placeholder.svg?height=300&width=300",
          ...
          <div className="grid gap-6 lg:grid-cols-[1fr_400px] lg:gap-12 xl:grid-cols-[1fr_600px]">
            <div className="flex flex-col justify-center space-y-4">
              ...
              <div className="flex flex-col gap-2 min-[400px]:flex-row">
                ...
                <Link href="/doctors">
                  <Button size="lg" variant="outline">Meet Our Doctors</Button>
                </Link>
              </div>
            </div>
            <div className="flex items-center justify-center">
              <div className="relative h-[300px] w-full md:h-[400px]">
                <Image src={department.image} alt={department.name} fill className="object-cover rounded-lg" />
              </div>
            </div>
          </div>
        </div>
            ...
            <TabsContent value="doctors" className="mt-6">
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {department.doctors.map((doctor, index) => (
                  <DoctorCard key={index} image={doctor.image} name={doctor.name} specialty={doctor.specialty} href={doctor.href} />
                ))}
              </div>
            </TabsContent>
          </Tabs>
